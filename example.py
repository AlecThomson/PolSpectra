...
for i in range(N_spectra):
    ...
    Ivalues = (
        np.random.lognormal(mean=-2, sigma=1, size=N_chan)
        * (freq_arr / np.mean(freq_arr)) ** -0.7
    )
    polarization = Faraday_thin_complex_polarization(
        freq_arr,
    ...
    Q_spectra.append(
        Ivalues * polarization.real + noise_amplitude * np.random.normal(0, 1, N_chan)
    )
    U_spectra.append(
        Ivalues * polarization.imag + noise_amplitude * np.random.normal(0, 1, N_chan)
    )
...
spectrumtable = polspectra.from_arrays(
    ra_array,
    dec_array,
    freq_arr,
    I_spectra,
...
spectrumtable.add_column(
    extra_column,
    name="random_integer",
    description="A random integer for each source",
    units="",
)

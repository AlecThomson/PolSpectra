...
class test_polspectra(unittest.TestCase):
    def test_01_create_from_lists(self):
        ...
        pol_spec = polspectra.from_arrays(
            source_arrays["ra"],
            source_arrays["dec"],
            source_arrays["freq"],
            source_arrays["stokesI"],
            source_arrays["stokesI_error"],
            source_arrays["stokesQ"],
            source_arrays["stokesQ_error"],
            source_arrays["stokesU"],
            source_arrays["stokesU_error"],
            source_arrays["source_number"],
            source_arrays["beam_maj"],
            source_arrays["beam_min"],
            source_arrays["beam_pa"],
        )
        self.assertIsInstance(
            pol_spec,
            polspectra.polarizationspectra,
            "Failed to create 2-row simple polarizationspectra.",
        )
        self.assertEqual(
            len(pol_spec),
            len(source_arrays["ra"]),
            "Simple polarizationspectra contains wrong number of rows.",
        )
        self.assertEqual(
            pol_spec.Nsrc,
            len(source_arrays["ra"]),
            "Simple polarizationspectra contains wrong number of sources.",
        )
        ...
        for colname in pol_spec.table.colnames:
            ...
            try:  # For channelized columns, check row by row for equivalence.
                ...
                for i in range(pol_spec.Nsrc):
                    self.assertTrue(
                        np.array_equiv(source_arrays[colname][i], pol_spec[colname][i]),
                        "{} Array not stored properly!".format(colname),
                    )
            except:  # For 1D columns, check entire column at once:
                self.assertTrue(
                    np.array_equiv(source_arrays[colname], pol_spec[colname]),
                    "{} Array not stored properly!".format(colname),
                )
    ...
    def test_02_simple_create_from_arrays(self):
        ...
        self.assertIsInstance(
            pol_spec,
            polspectra.polarizationspectra,
            "Failed to create table from arrays.",
        )
        self.assertEqual(
            len(pol_spec),
            len(source_arrays["ra"]),
        ...
        for colname in pol_spec.table.colnames:
            ...
            try:  # For channelized columns, check row by row for equivalence.
                ...
                for i in range(pol_spec.Nsrc):
                    if (
                        "beam" in colname
                    ):  # Beam column inputs are 1D, but table stores as 2D
                        self.assertTrue(
                            np.array_equiv(
                                source_arrays[colname], pol_spec[colname][i]
                            ),
                            "{} Array not stored properly!".format(colname),
                        )
                    else:
                        self.assertTrue(
                            np.array_equiv(
                                source_arrays[colname][i], pol_spec[colname][i]
                            ),
                            "{} Array not stored properly!".format(colname),
                        )
            except:  # For 1D columns, check entire column at once:
        ...
        self.assertTrue(
            np.array_equiv(pol_spec["freq"][-1], source_arrays["freq"]),
            "Frequency array not expanded from 1D properly",
        )
    ...
    def test_03_creation_from_variable_length_arrays(self):
        ...
        self.assertIsInstance(
            pol_spec,
            polspectra.polarizationspectra,
            "Failed to create 2-row different-channels polarizationspectra.",
        )
        self.assertEqual(
            len(pol_spec),
            len(source_arrays["ra"]),
        ...
        source_arrays["Nchan"] = [
            len(source_arrays["freq"][i]) for i in range(len(source_arrays["ra"]))
        ]
        for colname in pol_spec.table.colnames:
    ...
    def test_04_merging_basic_tables(self):
        ...
        pol_spec1 = polspectra.from_arrays(
            source_arrays1["ra"],
            source_arrays1["dec"],
            source_arrays1["freq"],
            source_arrays1["stokesI"],
            source_arrays1["stokesI_error"],
            source_arrays1["stokesQ"],
            source_arrays1["stokesQ_error"],
            source_arrays1["stokesU"],
            source_arrays1["stokesU_error"],
            source_arrays1["source_number"],
            source_arrays1["beam_maj"],
            source_arrays1["beam_min"],
            source_arrays1["beam_pa"],
        )
        ...
        pol_spec2 = polspectra.from_arrays(
            ...
            source_arrays2["dec"],
            source_arrays2["freq"],
            source_arrays2["stokesI"],
            source_arrays2["stokesI_error"],
            source_arrays2["stokesQ"],
            source_arrays2["stokesQ_error"],
            source_arrays2["stokesU"],
            source_arrays2["stokesU_error"],
            source_arrays2["source_number"],
            source_arrays2["beam_maj"],
            source_arrays2["beam_min"],
        ...
        pol_spec_merge.merge_tables(
            pol_spec2, merge_type="exact", source_numbers="concat"
        )
        self.assertIsInstance(
            pol_spec_merge,
            polspectra.polarizationspectra,
            "Failed to create simple merged polarizationspectra.",
        )
        self.assertEqual(
            pol_spec_merge.Nrows,
            pol_spec1.Nrows + pol_spec2.Nrows,
            "Merged version does not have expected number of rows!",
        )
        self.assertEqual(
            pol_spec_merge.Nsrc,
            pol_spec1.Nsrc + pol_spec2.Nsrc,
            "Merged (concat) version does not have expected number of rows!",
        )
        # Create merged version with overlapping sources:
        pol_spec_merge = pol_spec1.copy()
        pol_spec_merge.merge_tables(
            pol_spec2, merge_type="exact", source_numbers="keep"
        )
        self.assertIsInstance(
            pol_spec_merge,
            polspectra.polarizationspectra,
        ...
        self.assertEqual(
            pol_spec_merge.Nsrc,
            pol_spec1.Nsrc,
            "Merged (keep) version does not have expected number of rows!",
        )
    ...
    def test_05_readwrite_FITS(self):
        ...
        self.assertTrue(
            os.path.exists("./tests/simdata/test05.fits"),
            "FITS file has not been saved!",
        )
        readin_polspec = polspectra.from_FITS("./tests/simdata/test05.fits")
        self.assertEqual(
            readin_polspec.table.colnames,
            pol_spec2.table.colnames,
            "Table column names not preserved under FITS read/write.",
        )
        self.assertEqual(
            readin_polspec.Nrows,
            pol_spec2.Nrows,
            "Number of rows not preserved under FITS read/write",
        )
        self.assertEqual(
            readin_polspec.Nsrc,
            pol_spec2.Nsrc,
            "Number of sources not preserved under FITS read/write",
        )
        self.assertTrue(
            np.array_equal(pol_spec2.table.as_array(), readin_polspec.table.as_array()),
            "Table contents are not preserved under FITS read/write",
        )
    ...
    def test_06_optional_columns(self):
        ...
        source_arrays2["Nchan"] = [
            len(source_arrays2["freq"][i]) for i in range(pol_spec2.Nrows)
        ]
        for colname in [
            "stokesV",
            "stokesV_error",
            ...
            try:  # For channelized columns, check row by row for equivalence.
                ...
                for i in range(pol_spec2.Nsrc):
                    self.assertTrue(
                        np.array_equiv(
                            source_arrays2[colname][i], pol_spec2[colname][i]
                        ),
                        "{} Array not stored properly!".format(colname),
                    )
            except:  # For 1D columns, check entire column at once:
                self.assertTrue(
                    np.array_equiv(source_arrays2[colname], pol_spec2[colname]),
                    "{} Array not stored properly!".format(colname),
                )
        for colname in ["leakage", "channel_width"]:
            for i in range(pol_spec2.Nsrc):
                ...
                x[:] = [
                    [x]
                    for x in polspectra._possible_scalar_to_1D(
                        source_arrays2[colname], len(source_arrays2["ra"])
                    )
                ]
                self.assertTrue(
                    np.array_equiv(x, pol_spec2[colname]),
                    "{} Array not stored properly!".format(colname),
                )
    ...
    def test_07_expand_scalars_to_columns(self):
        ...
        self.assertIsInstance(
            pol_spec2,
            polspectra.polarizationspectra,
            "Failed to generate output when converting scalars to columns.",
        )
        self.assertTrue(
            pol_spec2["beam_pa"].data[-1] == [source_arrays2["beam_pa"]],
            "Beam columns were not accurately converted to column",
        )
        self.assertTrue(
            pol_spec2["ionosphere"][-1] == source_arrays2["ionosphere"],
            "String scalar not successfully expanded to column.",
        )
        self.assertTrue(
            pol_spec2["epoch"][-1] == source_arrays2["epoch"],
    ...
    def test_08_unspecified_columns(self):
        ...
        scalar = 7.0
        pol_spec2.add_column(
            values=scalar,
            name="scalar_column",
            description="Test scalar column",
            units="m/s",
        )
        self.assertTrue(
            "scalar_column" in pol_spec2.table.colnames,
            "Custom scalar column not added to table!",
        )
        self.assertEqual(
            pol_spec2["scalar_column"][0],
            scalar,
            "Custom scalar column values not propagated into table!",
        )
        ...
        self.assertTrue(
            "vector_column" in pol_spec2.table.colnames,
            "Custom vector column not added to table!",
        )
        self.assertTrue(
            np.array_equiv(pol_spec2["vector_column"], vector_col),
            "Custom vector column values not propagated into table!",
        )

        # It should fail if given vector of the wrong length:
        vector_col = [13, 6, 16]
        with self.assertRaises(
            ValueError, msg="Adding custom column allowed column of wrong size!"
        ):
            pol_spec2.add_column(
                values=vector_col,
                name="vector_column_wrongsize",
                description="Test vector column 2",
                units="kg",
            )

        # Add channel column with correct shape:
        channelized_column = [[0, 1], [0, 1, 2]]
        pol_spec2.add_channel_column(
            channelized_column,
            name="Channel_column",
            description="Test channel-wise column",
            units="",
        )
        self.assertTrue(
            "Channel_column" in pol_spec2.table.colnames,
            "Custom channel-wise column not added to table!",
        )
        self.assertTrue(
            np.array_equiv(pol_spec2["Channel_column"], channelized_column),
            "Custom channel-wise column values not propagated into table!",
        )

        # Should fail if incorrect number of channels:
        channelized_column = [[0, 1], [0, 1]]
        with self.assertRaises(
            Exception, msg="Adding custom column allowed column of wrong size!"
        ):
            pol_spec2.add_channel_column(
                channelized_column,
                name="Channel_column_wrong",
                description="Test channel-wise column",
                units="",
            )
        ...
        self.assertTrue(
            os.path.exists("./tests/simdata/test08.fits"),
            "Custom column FITS file has not been saved!",
        )
        readin_polspec = polspectra.from_FITS("./tests/simdata/test08.fits")
        self.assertEqual(
            readin_polspec.table.colnames,
            pol_spec2.table.colnames,
            "Custom columns lost in write-read process",
        )
        for colname in pol_spec2.table.colnames:
            try:  # For channelized columns, check row by row for equivalence.
                ...
                for i in range(pol_spec2.Nsrc):
                    self.assertTrue(
                        np.array_equiv(
                            readin_polspec[colname][i], pol_spec2[colname][i]
                        ),
                        "{} Array not stored properly!".format(colname),
                    )
            except:  # For 1D columns, check entire column at once:
                self.assertTrue(
                    np.array_equiv(readin_polspec[colname], pol_spec2[colname]),
                    "{} Array not stored properly!".format(colname),
                )
    ...
    def test_09_single_source_table(self):
        ...
        self.assertIsInstance(
            pol_spec1,
            polspectra.polarizationspectra,
            "Failed to create 1-row simple polarizationspectra.",
        )
    ...
    def test_10_merge_different_length_string_columms(self):
        ...
        pol_spec1 = polspectra.from_arrays(
            ...
            source_arrays1["beam_min"],
            source_arrays1["beam_pa"],
            telescope=source_arrays1["telescope"],
            cat_id=source_arrays1["source_name"],
        )
        source_arrays2 = make_simdata_1()
        source_arrays2["telescope"] = "Simulation long string"
        source_arrays2["source_name"] = [
            "Source 1 with long name",
            "Source 2 with long name",
        ]
        pol_spec2 = polspectra.from_arrays(
            source_arrays2["ra"],
            source_arrays2["dec"],
            ...
            source_arrays2["beam_pa"],
            telescope=source_arrays2["telescope"],
            cat_id=source_arrays2["source_name"],
        )
        pol_spec_merge = pol_spec1.copy()
        pol_spec_merge.merge_tables(
            pol_spec2, merge_type="exact", source_numbers="concat"
        ...
        self.assertIsInstance(
            pol_spec_merge,
            polspectra.polarizationspectra,
            "Failed to create merged table with different string lengths.",
        )
        self.assertEqual(
            pol_spec_merge.Nsrc,
            pol_spec1.Nsrc + pol_spec2.Nsrc,
        ...
        self.assertEqual(
            pol_spec_merge["telescope"][2],
            source_arrays2["telescope"],
            "Merged string column has incorrect entries!",
        )
        self.assertEqual(
            pol_spec_merge["cat_id"][2],
            source_arrays2["source_name"][0],
            "Merged string column has incorrect entries!",
        )
    ...
    def test_11_extract_subset_and_reindex(self):
        ...
        self.assertIsInstance(
            pol_spec1[0],
            polspectra.polarizationspectra,
            "Single-row selection does not return polarizationspectra object.",
        )
        self.assertIsInstance(
            pol_spec1["Nchan"],
            at.column.Column,
            "Column access does not return astropy.table Column object!",
        )
        ...
        self.assertTrue(
            len(source_zero) == 2, "Incorrectly extracted multi-row single source."
        )
        disordered_subselection = pol_spec_merge[[3, 5, 0]]
        disordered_subselection.renumber_sources()
        self.assertTrue(
            np.array_equiv(
                disordered_subselection["source_number"],
                range(len(disordered_subselection)),
            ),
            "Attempt at re-ordering source numbers has failed!",
        )
    ...
    def test_13_readwrite_HDF5(self):
        ...
        pol_spec2.table.add_column(
            pol_spec2.table["source_number"].astype(str), name="cat_id", index=0
        )
        pol_spec2.write_HDF5("./tests/simdata/test13.hdf5")
        self.assertTrue(
            os.path.exists("./tests/simdata/test13.hdf5"),
            "HDF5 file has not been saved!",
        )
        readin_polspec = polspectra.from_HDF5("./tests/simdata/test13.hdf5")
        self.assertEqual(
            sorted(readin_polspec.table.colnames),
            sorted(pol_spec2.table.colnames),
            "Table column names not preserved under HDF5 read/write.",
        )
        self.assertEqual(
            readin_polspec.Nrows,
            pol_spec2.Nrows,
            "Number of rows not preserved under HDF5 read/write",
        )
        self.assertEqual(
            readin_polspec.Nsrc,
            pol_spec2.Nsrc,
            "Number of sources not preserved under HDF5 read/write",
        )
        for col in pol_spec2.table.colnames:
            if pol_spec2.table[col].dtype == object:
                for i, j in zip(pol_spec2.table[col], readin_polspec.table[col]):
                    self.assertTrue(
                        np.array_equal(i, j),
                        "Object column values not preserved under HDF5 read/write.",
                    )
            else:
                self.assertTrue(
                    np.array_equal(
                        pol_spec2.table[col].data, readin_polspec.table[col].data
                    ),

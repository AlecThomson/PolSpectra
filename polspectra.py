...
class polarizationspectra:
    ...
    def create_from_arrays(
        ...
        stokesQ_error_column = Column(
            name="stokesQ_error",
            shape=(),
            length=self.Nrows,
            dtype="object",
            description="Stokes Q error per channel",
        )
        stokesQ_error_column[:] = [x for x in stokesQ_error]

        stokesU_column = Column(
            name="stokesU",
            dtype="object",
            shape=(),
            length=self.Nrows,
        ...
        stokesU_error_column = Column(
            name="stokesU_error",
            shape=(),
            length=self.Nrows,
            dtype="object",
            description="Stokes U error per channel",
        )
        stokesU_error_column[:] = [x for x in stokesU_error]

        source_number_column = Column(
            data=source_number_array,
            name="source_number",
            dtype="int",
        ...
        beam_maj_column[:] = [
            [x] if np.array(x).ndim == 0 else x
            for x in _possible_scalar_to_1D(beam_maj, self.Nrows)
        ]

        beam_min_column = Column(
            length=self.Nrows,
            name="beam_min",
            dtype="object",
            unit=u.deg,
        ...
        beam_min_column[:] = [
            [x] if np.array(x).ndim == 0 else x
            for x in _possible_scalar_to_1D(beam_min, self.Nrows)
        ]

        beam_pa_column = Column(
            length=self.Nrows,
            name="beam_pa",
            dtype="object",
            unit=u.deg,
        ...
        beam_pa_column[:] = [
            [x] if np.array(x).ndim == 0 else x
            for x in _possible_scalar_to_1D(beam_pa, self.Nrows)
        ]
        ...
        self.table = Table(
            [
                ...
                freq_column,
                stokesI_column,
                stokesI_error_column,
                stokesQ_column,
                stokesQ_error_column,
                stokesU_column,
                stokesU_error_column,
                beam_maj_column,
                beam_min_column,
                beam_pa_column,
                Nchan_column,
        ...
        if stokesV is not None:  # Check that both Stokes V and error are supplied?
            ...
            stokesV_error_column = Column(
                name="stokesV_error",
                shape=(),
                length=self.Nrows,
                dtype="object",
                description="Stokes V error per channel",
            )
        ...
        if cat_id is not None:
            source_name_column = Column(
                data=cat_id, name="cat_id", dtype="str", description="Source name"
            )
            self.table.add_column(source_name_column)

        if dataref is not None:
            dataref_column = Column(
                data=_possible_scalar_to_1D(dataref, self.Nrows),
                name="dataref",
                dtype="str",
        ...
        if leakage is not None:  # Like the beam columns, could be scalar, 1D, or 2D.
            ...
            leakage_column[:] = [
                [x] if np.array(x).ndim == 0 else x
                for x in _possible_scalar_to_1D(leakage, self.Nrows)
            ]
            self.table.add_column(leakage_column)

        if (
            channel_width is not None
        ):  # Like the beam columns, could be scalar, 1D, or 2D.
            # Thus slightly complicated to deal with.
            channel_width_column = Column(
                length=self.Nrows,
            ...
            channel_width_column[:] = [
                [x] if np.array(x).ndim == 0 else x
                for x in _possible_scalar_to_1D(channel_width, self.Nrows)
            ]
    ...
    def add_channel_column(self, values, name, description, units=""):
        ...
        for i in range(self.Nrows):
            if len(values[i]) != self["Nchan"][i]:
                raise Exception(
                    "New column doesn't have same number of channels as rest of table, on row {}.".format(
                        i
                    )
                )
    ...
    def read_HDF5(self, filename):
        ...
        with h5py.File(filename, "r") as f:
            ...
            descs = ["Source name"] + [
                first_row[col].attrs["description"] for col in first_row.keys()
            ]
            dtypes = [str] + [
                first_row[col].dtype if first_row[col].shape == () else object
    ...
    def read_FITS(self, filename):
        ...
        for i in range(1, header["TFIELDS"] + 1):
            if "TUNIT{}".format(i) in header.keys():
                self.table[header["TTYPE{}".format(i)]].unit = header[
                    "TUNIT{}".format(i)
                ]
    ...
    def merge_tables(self, table2, merge_type="exact", source_numbers="keep"):
        ...
        if (source_numbers != "keep") and (source_numbers != "concat"):
            raise Exception(
                "Incorrect option given for source_number behaviour in table merging!"
            )
        ...
        add_table = (
            table2.table.copy()
        )  # Copy the 2nd table to avoid accidently writing to it.
        if source_numbers == "concat":  # Reset the numbering in the 2nd table
            ...
            add_table["source_number"] = (
                add_table["source_number"]
                - np.min(add_table["source_number"])
                + np.max(self.table["source_number"])
                + 1
            )

        merged_table = vstack(
            [self.table, add_table], join_type=merge_type, metadata_conflicts="error"
        )
    ...
    def renumber_sources(self):
        ...
        source_numbers, indices = np.unique(
            self.table["source_number"], return_index=True
        )
    ...
    def verify_table(self):
        ...
        if "leakage" in self.table.colnames:
            ...
            pass
        if ("channel_width" in self.table.colnames) and (
            self.table["channel_width"].dtype.type == np.object_
        ):
            for i in range(self.Nrows):
                if len(self.table[i]["channel_width"]) not in [
                    self.table[i]["Nchan"],
                    1,
                ]:
        ...
        if len(badvalues) == 0:
            ...
            print(
                "Some columns have mis-matched numbers of channels.\n"
        ...
        if np.min([np.min(x) for x in self.table["freq"]]) < 1e6:
            print(
                "Some frequency values are much smaller than expected."
        ...
        if np.max([np.max(x) for x in self.table["beam_maj"]]) > 1:
            print(
                "Some beam size values are much larger than expected."
...
def from_arrays(
    ...
    new_spectra.create_from_arrays(
        long_array,
        lat_array,
        freq_array,
        stokesI,
